...
import authenticate, { AuthResponse } from "./auth";
    ...
    await authenticate(req, _User)
      .then(({ authenticated, email }: AuthResponse) => {
        if (!authenticated) {
          ...
          const { place_id, name, lat, lng, address } = req.body;
    ...
    await authenticate(req, _User)
      .then(({ authenticated, email }: AuthResponse) => {
        if (!authenticated) {
          ...
          _User
            .saveConrifmMarzocco(place_id, email)
            .then((resq: DBResponse) => {
              res
                .status(resq.status)
                .json({ message: "success", data: resq.result });
